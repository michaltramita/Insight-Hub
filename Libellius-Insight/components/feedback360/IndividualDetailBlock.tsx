...
const IndividualDetailBlock: React.FC<Props> = ({ individual }) => {
  ...
  const activeIndex = useMemo(
    () =>
      individual.competencies.findIndex(
        (competency) => competency.id === activeCompetencyId
      ),
    [individual, activeCompetencyId]
  );
  const activeCompetency =
    activeIndex >= 0
      ? individual.competencies[activeIndex]
      : individual.competencies.length
      ? individual.competencies[0]
      : null;

  const goToRelativeCompetency = (direction: -1 | 1) => {
    if (!individual.competencies.length || activeIndex < 0) return;
    const nextIndex =
      (activeIndex + direction + individual.competencies.length) %
      individual.competencies.length;
    setActiveCompetencyId(individual.competencies[nextIndex].id);
  };
      ...
      <div className="bg-white rounded-[2rem] border border-black/5 p-6 md:p-8 shadow-2xl shadow-black/5">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xl font-black uppercase tracking-tight">
            Detailné výsledky - {individual.name}
          </h3>
        </div>
        <p className="text-black/60 font-semibold mt-3 max-w-4xl">
          Zobrazené sú tvrdenia pre aktívnu kompetenciu. Medzi kompetenciami sa prepínate cez posuvník.
        </p>
      </div>

      {individual.competencies.length > 0 && activeCompetency && (
        <div className="bg-white rounded-[2rem] border border-black/5 p-4 md:p-5 shadow-2xl shadow-black/5 space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-[10px] uppercase tracking-widest font-black text-black/45">
              Kompetencia {Math.max(1, activeIndex + 1)} / {individual.competencies.length}
            </p>
