...
import {
  ...
  ChevronLeft, ChevronRight, Printer // <-- PRIDANÁ IKONA TLAČIARNE
} from 'lucide-react';
import {
...
const SatisfactionDashboard: React.FC<Props> = ({ result, onReset }) => {
  ...
  // --- NOVÝ STAV PRE TLAČOVÝ MÓD ---
  const [isPrinting, setIsPrinting] = useState(false);

  // --- FUNKCIA NA TLAČ ---
  const handlePrint = () => {
    setIsPrinting(true);
    // Dáme Reactu čas 800ms na rozbalenie a vykreslenie všetkých grafov
    setTimeout(() => {
      window.print();
      setIsPrinting(false);
    }, 800);
  ...
  const renderSection = (areaId: string) => {
    ...
    return (
      <div className="space-y-8 sm:space-y-10 animate-fade-in print:break-inside-avoid">
            ...
            {viewMode === 'DETAIL' && (
              <div className="flex flex-col items-start lg:items-end gap-3 w-full lg:w-auto">
                ...
                <div className="relative w-full lg:w-auto lg:min-w-[340px]">
                  ...
                  <ChevronDown className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2 w-5 h-5 sm:w-6 sm:h-6 text-white/40 pointer-events-none print:hidden" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-8 sm:space-y-10">
            <div className="bg-white p-6 sm:p-8 md:p-10 lg:p-14 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] border border-black/5 shadow-2xl flex flex-col">
              <div className="mb-6 sm:mb-8 flex items-start gap-4">
                <div className="bg-brand/5 p-3 rounded-2xl flex-shrink-0 print:hidden">
              ...
              <div className="w-full print:h-auto">
                <div className="h-[450px] sm:h-[500px] lg:h-[550px] w-full print:h-[600px]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-10 print:grid-cols-2">
              <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] border border-black/5 shadow-2xl print:break-inside-avoid">
                <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 lg:mb-10 text-brand">
                  <Star className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 print:hidden" />
              ...
              <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] border border-black/5 shadow-2xl print:break-inside-avoid">
                <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 lg:mb-10 text-black">
                  <Target className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 print:hidden" />
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 sm:px-6 lg:px-8 bg-white print:p-0">
      <div className="flex-1 w-full max-w-[1600px] 2xl:max-w-[1800px] mx-auto flex flex-col">
        <div className="space-y-6 sm:space-y-8 animate-fade-in pb-10 sm:pb-12 print:space-y-10">

      <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] border border-black/5 p-5 sm:p-8 md:p-10 lg:p-12 shadow-2xl flex flex-col xl:flex-row justify-between items-start gap-6 sm:gap-8 relative overflow-hidden print:shadow-none print:border-b-2 print:border-black/10 print:rounded-none">
        <div className="flex flex-col gap-4 sm:gap-6 relative z-10 w-full xl:w-auto min-w-0">
          <div className="space-y-2 sm:space-y-3">
            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 bg-brand/5 rounded-full border border-brand/10 w-fit print:hidden">
          ...
          <div className="w-16 h-1 bg-black/5 rounded-full print:hidden"></div>
          ...
          <div className="space-y-2 sm:space-y-3 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-2">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-black/5 rounded-lg border border-black/5 min-w-0 print:border-none print:bg-transparent print:px-0">
                <Building2 className="w-4 h-4 text-black/40 shrink-0 print:hidden" />
                <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-black/60 truncate print:text-black">
                  {data.clientName || 'Názov firmy'}
                </span>
              </div>
              <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-black/30 print:text-black/50">
        ...
        <div className="flex flex-col items-stretch gap-2 sm:gap-3 relative z-10 w-full xl:w-auto xl:min-w-[220px] xl:items-end shrink-0 pt-1 sm:pt-2 md:pt-4 xl:pt-0 print:hidden">
          {!isSharedView && (
            <>
              {/* TLAČIDLO PRE PDF REPORT */}
              <button
                onClick={handlePrint}
              ...
              <button
                ...
                className="w-full xl:w-[220px] flex items-center justify-center gap-2 px-4 sm:px-6 py-3 sm:py-4 bg-white border border-black/10 text-black hover:bg-black hover:text-white rounded-xl sm:rounded-2xl font-black transition-all text-[10px] sm:text-[11px] uppercase tracking-widest shadow-xl"
              >
                <Download className="w-4 h-4" /> Export dát
              </button>
        ...
        <div className="absolute top-[-20%] right-[-10%] w-72 sm:w-96 h-72 sm:h-96 bg-brand/5 rounded-full blur-[100px] pointer-events-none -z-0 print:hidden"></div>
      </div>
      ...
      {/* ZMENA V PODMIENKE ZOBRAZENIA ZÁLOŽIEK: Ukážeme ju buď ak je aktívna, ALEBO ak tlačíme */}
      {(activeTab === 'ENGAGEMENT' || isPrinting) && (
        <div className="space-y-8 sm:space-y-10 animate-fade-in print:break-before-page">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="bg-white border border-black/5 p-6 sm:p-8 lg:p-10 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] shadow-2xl transition-transform hover:scale-[1.02] print:border-black/20">
              ...
              <table className="w-full min-w-[760px] text-left">
                ...
                <tbody className="divide-y divide-black/5 font-black text-sm">
                  {filteredEngagement.length > 0 ? filteredEngagement.map((team: any, idx: number) => (
                    <tr key={idx} className={`hover:bg-brand/5 transition-colors group ${team.name.toLowerCase().includes('priemer') ? 'bg-brand/5 text-brand print:bg-brand/5' : ''}`}>
                      ...
                      <td className="p-4 sm:p-7">
                        <div className="flex items-center justify-center gap-4 sm:gap-5">
                          <div className="w-28 sm:w-40 bg-black/5 h-2.5 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-brand shadow-[0_0_10px_rgba(184,21,71,0.3)] print:shadow-none"
          ...
          {filteredEngagement.length > 0 && (
            <div className="bg-white p-6 sm:p-8 md:p-10 lg:p-12 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] border border-black/5 shadow-2xl animate-fade-in print:break-inside-avoid">
                ...
                {engagementVisualMode === 'CARDS' ? (
                  <div className="relative">
                    {engagementTeamCards.length > 2 && (
                      <>
                        <div
                          className={`hidden sm:block absolute left-0 top-0 bottom-8 w-10 bg-gradient-to-r from-white to-transparent z-10 pointer-events-none transition-opacity print:hidden ${
                            canScrollEngagementLeft ? 'opacity-100' : 'opacity-0'
                          }`}
                        />
                        <div
                          className={`hidden sm:block absolute right-0 top-0 bottom-8 w-10 bg-gradient-to-l from-white to-transparent z-10 pointer-events-none transition-opacity print:hidden ${
                    ...
                    <div
                      ...
                      className="flex items-start gap-4 sm:gap-5 overflow-x-auto pb-2 pr-1 snap-x snap-mandatory no-scrollbar print:flex-wrap print:overflow-visible print:gap-y-6"
                    >
                      {engagementTeamCards.map((team: any, idx: number) => {
                        const cardId = `${team.name}-${idx}`;
                        // V tlačovom móde rozbalíme pre istotu všetky interpretácie
                        const isExpanded = isPrinting || expandedEngagementCard === cardId;

                        return (
                          <div
                            key={cardId}
                            className={`snap-start self-start shrink-0 w-[92%] sm:w-[calc(50%-10px)] min-w-[320px] min-h-[380px] lg:min-h-[430px] rounded-2xl sm:rounded-3xl border p-4 sm:p-5 lg:p-6 print:w-full print:break-inside-avoid ${
                          ...
                          >
                            ...
                            <div className="mt-4 pt-3 border-t border-black/5">
                              <button
                                ...
                                className="w-full flex items-center justify-between rounded-xl px-2 py-2.5 hover:bg-white/70 transition-colors print:hidden"
                              >
                              ...
                              {isExpanded && (
                                <div className="mt-3 space-y-3 animate-fade-in print:mt-4">
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 sm:gap-8 items-start xl:items-center">
                    ...
                    <div className="xl:col-span-5 w-full">
                      <div className="bg-black/5 rounded-2xl sm:rounded-3xl border border-black/5 p-4 md:p-5">
                        ...
                        <div className="space-y-3 max-h-[340px] overflow-auto pr-1 print:max-h-none print:overflow-visible">
                          {engagementChartData
                            .slice()
                            .sort((a: any, b: any) => {
                              if (a.isActive && !b.isActive) return -1;
                            ...
                            .map((team: any, idx: number) => (
                              <div
                                key={`${team.name}-${idx}`}
                                className={`rounded-2xl border p-3 sm:p-4 transition-all print:break-inside-avoid ${
                                  team.isActive 
                                    ? (idx === 0 ? 'bg-brand/5 border-brand/20' : 'bg-white border-black/5') 
                                    : 'bg-black/5 border-transparent opacity-50 grayscale print:opacity-100'
                                }`}
                              >
      ...
      {/* ZMENA V PODMIENKE ZOBRAZENIA ZÁLOŽIEK: Ukážeme ju buď ak je aktívna, ALEBO ak tlačíme */}
      {(activeTab === 'OPEN_QUESTIONS' || isPrinting) && (
        <div className="space-y-8 sm:space-y-10 animate-fade-in print:break-before-page">
          <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] border border-black/5 shadow-2xl">
                ...
                <div className="w-full">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full p-4 sm:p-5 pr-12 bg-black text-white rounded-[1rem] sm:rounded-[1.5rem] font-black text-base sm:text-lg outline-none shadow-xl cursor-pointer hover:bg-brand transition-all appearance-none tracking-tight print:bg-white print:text-black print:border print:border-black/10"
                    >
                      {masterTeams.map((t: string) => <option key={t} value={t}>{t}</option>)}
                    </select>
                    <ChevronDown className="absolute right-5 top-1/2 -translate-y-1/2 w-5 h-5 text-white/40 pointer-events-none print:hidden" />
                  </div>
                </div>
                ...
                <div className="w-full">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full p-4 sm:p-5 pr-12 bg-black/5 text-black rounded-[1rem] sm:rounded-[1.5rem] font-bold text-sm outline-none shadow-sm cursor-pointer border border-black/5 hover:bg-black/10 transition-all appearance-none print:bg-white"
                    ...
                    <ChevronDown className="absolute right-5 top-1/2 -translate-y-1/2 w-5 h-5 text-black/40 pointer-events-none print:hidden" />
                  </div>
                </div>
              </div>
            </div>
          ...
          {selectedQuestionData?.recommendations && selectedQuestionData.recommendations.length > 0 ? (
            <div className="flex flex-col gap-4 sm:gap-6">
              {selectedQuestionThemeCloud.length > 0 && (
                <div className="bg-white p-6 sm:p-8 md:p-10 rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[2.5rem] border border-black/5 shadow-xl print:break-inside-avoid">
                  <h5 className="text-[11px] font-black uppercase tracking-[0.2em] text-brand mb-4 flex items-center gap-2">
                    <Lightbulb className="w-4 h-4 print:hidden" /> Tematická mapa odpovedí (otázka)
                  </h5>
                  ...
                  <div className="bg-black/5 rounded-2xl p-4 sm:p-5 md:p-6 border border-black/5">
                    <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-4 gap-y-2 sm:gap-y-3">
                      ...
                      <span
                          ...
                          className={`inline-flex items-center rounded-xl px-3 py-1.5 font-black tracking-tight cursor-help select-none transition-all ${tIdx < 2 ? 'text-brand bg-brand/10' : 'text-black bg-white'} ${getThemeFontSizeClass(theme.count, selectedQuestionMaxThemeCount)} hover:scale-[1.03]`}
                        >
                          {theme.theme}
                        </span>
                      ))}
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-black/35 mt-4 print:hidden">
              ...
              {selectedQuestionData.recommendations.map((rec: any, index: number) => {
                ...
                // V tlačovom móde sú všetky kartičky rozbalené automaticky
                const isExpanded = isPrinting || expandedRecIndex === index;

                return (
                  <div
                    key={index}
                    className={`bg-white p-5 sm:p-6 md:p-8 lg:p-10 rounded-[1.25rem] sm:rounded-[1.75rem] lg:rounded-[2.5rem] border transition-all duration-300 flex flex-col group cursor-pointer print:break-inside-avoid ${isExpanded ? 'border-brand/20 shadow-2xl' : 'border-black/5 shadow-xl hover:shadow-2xl hover:border-black/10'}`}
                  ...
                  >
                    <div className="flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-8 items-start w-full">
                      ...
                      <div className="flex-grow pt-1 sm:pt-2 flex flex-col md:flex-row justify-between items-start gap-4 min-w-0">
                        ...
                        <div className={`shrink-0 mt-1 md:mt-2 w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center bg-black/5 transition-transform duration-300 print:hidden ${isExpanded ? 'rotate-180 bg-brand/10 text-brand' : 'text-black/40 group-hover:bg-black/10'}`}>
                        ...
                        {hasQuotes ? (
                          <div>
                            <h5 className="text-[11px] font-black uppercase tracking-[0.2em] text-brand mb-4 sm:mb-6 flex items-center gap-2">
                              <MessageCircle className="w-4 h-4 print:hidden" /> Reprezentatívne citácie z odpovedí
                            </h5>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                              {rec.quotes.map((quote: string, qIdx: number) => (
                                <div key={qIdx} className="bg-black/5 p-4 sm:p-5 rounded-2xl relative print:bg-white print:border print:border-black/5">
                          ...
                          <div className="bg-black/5 rounded-2xl p-4 sm:p-5 text-sm font-bold text-black/50 print:bg-transparent">
      ...
      {/* ZMENA V PODMIENKE ZOBRAZENIA OBLASTÍ: Ak tlačíme, zbehneme .map a vyrendrujeme VŠETKY oblasti */}
      {isPrinting ? (
        (data.areas || []).map((area: any) => (
          <div key={`print-${area.id}`} className="print:mt-12 print:break-before-page">
            {renderSection(area.id)}
          </div>
        ))
      ) : (
      ...
      <div className="mt-12 sm:mt-16 pt-8 sm:pt-10 border-t border-black/10 flex flex-col md:flex-row justify-between items-center gap-4 sm:gap-6 text-black/40 pb-4 sm:pb-6 print:mt-8">
        <div className="flex items-center gap-4">
