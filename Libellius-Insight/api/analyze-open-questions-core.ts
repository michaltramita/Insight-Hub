...
const normalizeForThemeCompare = (value: string) =>
  String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
const isExcludedTheme = (theme: string) => {
  const normalized = normalizeForThemeCompare(theme);
...
function buildThemeStats(answers: Array<string | RawAnswer>) {
  ...
  for (const ans of answers || []) {
    ...
    if (!tema || isExcludedTheme(tema)) continue;
...
export async function analyzeOpenQuestionsPayload(
  rawOpenQuestionsForAI: unknown,
  apiKey: string
) {
  if (!rawOpenQuestionsForAI || !Array.isArray(rawOpenQuestionsForAI)) {
    throw new Error("Missing or invalid rawOpenQuestionsForAI");
  }
...
`.trim();
      ...
      questions: (team?.questions || []).map((q: any) => {
        ...
        const normalizedThemeCloud =
          Array.isArray(meta?.themeStats) && meta!.themeStats.length > 0
            ? meta!.themeStats
                .filter((t: any) => t?.theme && !isExcludedTheme(String(t.theme)))
                .map((t: any) => ({
  ...
  return { openQuestions: parsed.openQuestions };
}
