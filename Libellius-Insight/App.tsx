...
const App: React.FC = () => {
  ...
  const handleDecryptSharedReport = async () => {
    ...
    try {
      const jsonData: any = await decryptReportFromUrlPayload(pendingEncryptedPayload, sharePassword.trim());
      if (!jsonData.mode) {
        jsonData.mode = jsonData.satisfaction ? 'ZAMESTNANECKA_SPOKOJNOST' : '360_FEEDBACK';
      }
      setResult(jsonData);
        ...
        {!pendingEncryptedPayload && status === AppStatus.HOME && !showSharedGoodbye && (
          <div className="flex flex-col items-center justify-center flex-grow text-center animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 w-full max-w-5xl px-0 sm:px-2">
              <div className="group p-6 sm:p-8 md:p-10 border-2 border-black/5 rounded-[2rem] md:rounded-[2.5rem] text-left flex flex-col items-start gap-5 md:gap-6 shadow-xl shadow-black/5 relative overflow-hidden bg-[#f9f9f9] opacity-85 cursor-not-allowed">
                ...
                <div className="z-10 mt-2 md:mt-4 w-full sm:w-auto px-6 sm:px-8 md:px-10 py-3.5 md:py-4 bg-black/10 text-black/40 rounded-full font-black text-xs sm:text-sm uppercase tracking-widest text-center border border-black/10">PRIPRAVUJEME</div>
              </div>
              <button
              ...
              >
                ...
                <div className="z-10 mt-2 md:mt-4 w-full sm:w-auto px-6 sm:px-8 md:px-10 py-3.5 md:py-4 bg-brand text-white rounded-full font-black text-xs sm:text-sm uppercase tracking-widest shadow-lg transform active:scale-95 transition-all text-center">VYBRAŤ TENTO MÓD</div>
              </button>
            </div>
          </div>
        )}
