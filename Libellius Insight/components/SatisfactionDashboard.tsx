...
// Typy pre navigáciu používajú anonymné kľúče card1-4
...
const SatisfactionDashboard: React.FC<Props> = ({ result, onReset }) => {
  ...
  // Stavy pre vybrané tímy na jednotlivých kartách
  const [selectedTeams, setSelectedTeams] = useState<Record<string, string>>({
    card1: '', card2: '', card3: '', card4: ''
  });

  // Stavy pre porovnávacie matice
  ...
  // 1. Získanie zoznamu všetkých tímov z engagementu
  ...
  // Inicializácia vybraných tímov
  useEffect(() => {
    if (masterTeams.length > 0 && !selectedTeams.card1) {
      const initial = masterTeams[0];
